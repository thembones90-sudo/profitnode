...
/* PN CQ CONFIDENCE V8 START */
function pbComponentQualityCoverage(by){
  let total=0,rated=0,verified=0,estimated=0;
  RIG_SLOTS.forEach(k=>{
    const slotW="STORAGE"===k
      ?PB_CQ_SLOT_WEIGHTS.STORAGE
      :(typeof isStorageSlot==="function"&&isStorageSlot(k)?PB_EXTRA_DRIVE_WEIGHT:(PB_CQ_SLOT_WEIGHTS[k]||0));
    const r=by&&by[k];
    if(!slotW||!r||r.missing||!r.label)return;
    total++;
  ...
  return{rated:rated,total:total,verified:verified,estimated:estimated,missing:Math.max(0,total-rated)};
}
function pbConfidenceDisplay(conf,detail){
  if(conf==="PARTIAL"&&detail&&Number.isFinite(detail.rated)&&Number.isFinite(detail.total)){
    return"PARTIAL DATA · "+detail.rated+"/"+detail.total+" RATED";
  }
  return PB_CONF_LABEL[conf]||PB_CONF_LABEL.UNVERIFIED;
}
/* PN CQ CONFIDENCE V8 END */
...
function pbRatingConfidences(categories,by,perf,val){
  ...
  const cqCoverage=pbComponentQualityCoverage(by);
  out.COMPONENT_QUALITY=categories.COMPONENT_QUALITY===null
    ?"UNRATED"
    :(cqCoverage.rated<cqCoverage.total
      ?"PARTIAL"
...
function pbRatingRowHtml(k,hooks,label,score,conf,detail){
