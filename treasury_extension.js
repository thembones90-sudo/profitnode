...
function pnTreasurySourceCards(e){
  ...
  return'<section class="pn-wc-sources"><div class="pn-wc-section-head"><div><span>LIQUID SOURCES</span><b>THE FIVE RESERVES</b></div><small>Native balance · EUR equivalent</small></div><div class="pn-wc-source-grid">'+PN_TREASURY_CORE_BALANCES.map(def=>{
    const b=t.balances.find(x=>x.sourceKey===def.sourceKey)||{amount:0,currency:def.currency,include:!0},enabled=b.include!==!1;
    return'<article class="pn-wc-source '+(enabled?'':'is-muted')+'" data-wc-source="'+def.sourceKey+'"><div class="pn-wc-source-mark"><img src="'+PN_WC_SOURCE_ASSETS[def.sourceKey]+'" alt="" aria-hidden="true"></div><div><span>'+escHtml(def.label)+'</span><strong>'+pnTreasuryNativeMoney(b.amount,b.currency||def.currency)+'</strong><small>'+pnTreasuryMoney(pnTreasuryEur(b.amount,b.currency||def.currency,settings))+(enabled?'':' · excluded')+'</small></div></article>'
  }).join("")+'</div></section>'
}
...
function pnTreasuryMovements(e,t,r,s){
  return'<section class="pn-wc-movements panel"><div class="pn-wc-section-head"><div><span>NEXT MOVEMENT</span><b>FORECAST VECTOR</b></div><small>'+escHtml(s)+'</small></div><div class="pn-wc-movement-grid">'
    +pnTreasuryCard("Obligations",pnTreasuryMoney(t.obligations),e.obligations.filter(x=>"PAID"!==x.status).length+" pending","is-dues"+(pnTreasuryZero(t.obligations)?" is-zero":""),"chain")
    +pnTreasuryCard("Pending Conversion",pnTreasuryMoney(t.pending),e.pendingAssets.filter(x=>!x.converted).length+" unconverted","is-spoils"+(pnTreasuryZero(t.pending)?" is-zero":""),"crate")
    +pnTreasuryCard("Incoming Tribute",pnTreasuryMoney(t.income),r?((r.source||"Tribute source")+" · "+pnTreasuryConfidence(r.confidence)):"No tribute projected","is-tribute"+(pnTreasuryZero(t.income)?" is-zero":""),"crown")
...
function pnTreasuryActivityPanel(e){const n=(e.flows||[]).length,filter=state.treasuryFlowFilter||"ALL",filters=["ALL","INCOME","SPENDING","SALES","ADJUSTMENTS"];return'<section class="panel pn-treasury-activity"><div class="panel-head"><h2>War Chest Ledger</h2><span>'+n+" recorded flow"+(1===n?"":"s")+'</span></div><div class="pn-wc-ledger-tools">'+filters.map(x=>'<button type="button" class="'+(filter===x?'is-active':'')+'" data-treasury-flow-filter="'+x+'">'+x+'</button>').join("")+'</div><div class="panel-body">'+pnTreasuryActivityRows(e)+'</div>'+(n>5?'<button type="button" class="pn-wc-ledger-more" data-treasury-ledger-toggle>'+(state.treasuryLedgerExpanded?'SHOW LATEST FIVE':'VIEW FULL LEDGER')+'</button>':'')+'</section>'}
function renderTreasury(){
  ...
  const meta='<div class="pn-treasury-strip"><span>LAST RECOUNTED <b>'+pnTreasuryLastRecount(e)+'</b></span><span class="pn-wc-fx">PLANNING FX · USD '+Number(e.settings.usdToEur).toFixed(4)+' EUR · RSD '+Number(e.settings.rsdToEur).toFixed(5)+' EUR</span></div>';
  return pageHeader("War Chest","Private reserves · isolated from Shop funds",'<button type="button" class="btn btn-primary pn-wc-btn" data-treasury-new>RECOUNT THE HOARD</button>')+'<div class="content pn-treasury">'+meta+'<div class="pn-treasury-grid">'+pnTreasuryHero(t,e,a)+'</div>'+pnTreasurySourceCards(e)+pnTreasuryMovements(e,t,r,s)+pnTreasuryActivityPanel(e)+pnTreasuryHistory(e)+(state.treasuryDraft?pnTreasuryEditor():"")+'</div>'
}
...
(function(){
  ...
  .pn-wc-hero-surplus{font-weight:800}.pn-wc-hero-surplus.pos{color:var(--wc-green-lit)}.pn-wc-hero-surplus.neg{color:var(--wc-red)}
  ...
  .pn-treasury-editor{margin-top:10px}.pn-treasury-settings{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px}.pn-treasury-editor label{display:flex;flex-direction:column;gap:5px}.pn-treasury-editor label>span{font-family:var(--mono);font-size:9px;color:var(--wc-text-mute);text-transform:uppercase}.pn-treasury-editor input,.pn-treasury-editor select{width:100%;min-width:0;height:33px}.pn-treasury-fx-note{margin:7px 0 0;color:var(--wc-text-mute);font:10px var(--mono)}
  ...
  .pn-treasury-strip{padding:7px 12px;font-size:9px}.pn-treasury-strip span{display:flex;align-items:center;gap:6px}.pn-treasury-strip span b{font-size:10px;color:var(--wc-silver)}.pn-treasury-strip .pn-wc-fx{margin-left:auto;color:var(--wc-text-mute);letter-spacing:.03em}
  ...
  .pn-wc-gauge-fill{background-image:linear-gradient(90deg,var(--wc-silver),var(--wc-silver))!important}.pn-wc-gauge-floor{background:var(--wc-gold);box-shadow:0 0 8px var(--wc-glow-gold)}.pn-wc-gauge-floor::before{border-top-color:var(--wc-gold)}
  .pn-wc-section-head{min-height:42px;padding:9px 12px;display:flex;align-items:center;justify-content:space-between;gap:14px;border-bottom:1px solid var(--wc-border);background:linear-gradient(90deg,rgba(255,255,255,.025),transparent)}.pn-wc-section-head>div{display:flex;align-items:baseline;gap:10px}.pn-wc-section-head span,.pn-wc-section-head small{font:9px var(--mono);letter-spacing:.08em;color:var(--wc-text-mute)}.pn-wc-section-head b{font:800 12px var(--display);letter-spacing:.06em;color:var(--wc-silver)}
  .pn-wc-sources{margin:8px 0;border:1px solid var(--wc-border);background:var(--wc-panel)}.pn-wc-source-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr))}.pn-wc-source{min-width:0;padding:12px;display:grid;grid-template-columns:50px 1fr;gap:10px;align-items:center;border-left:1px solid var(--wc-border)}.pn-wc-source:first-child{border-left:0}.pn-wc-source.is-muted{opacity:.48}.pn-wc-source-mark{width:50px;height:34px;display:flex;align-items:center;justify-content:center;padding:5px;border:1px solid var(--wc-border);background:#0c0a0e}.pn-wc-source-mark img{display:block;max-width:100%;max-height:100%;object-fit:contain}.pn-wc-source[data-wc-source="PREPLY"] .pn-wc-source-mark img{filter:invert(1)}.pn-wc-source>div:last-child{min-width:0;display:flex;flex-direction:column}.pn-wc-source span{font:9px var(--mono);letter-spacing:.07em;text-transform:uppercase;color:var(--wc-text-mute)}.pn-wc-source strong{margin:3px 0;font:800 15px var(--mono);font-variant-numeric:tabular-nums;color:var(--wc-silver);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.pn-wc-source small{font:9px var(--mono);color:var(--wc-text-mute);white-space:nowrap}
  ...
  @media(max-width:760px){.pn-treasury-grid{grid-template-columns:1fr}.pn-treasury-strip{flex-wrap:wrap}.pn-treasury-strip .pn-wc-fx{margin-left:0}.pn-wc-source-grid,.pn-wc-movement-grid{grid-template-columns:1fr}.pn-wc-source,.pn-wc-source:nth-child(n),.pn-wc-movement-grid .pn-treasury-card:nth-child(n){border-left:0;border-top:1px solid var(--wc-border)}.pn-wc-source:first-child,.pn-wc-movement-grid .pn-treasury-card:first-child{border-top:0}.pn-wc-section-head{align-items:flex-start}.pn-wc-section-head>div{display:block}.pn-wc-section-head b{display:block;margin-top:3px}.pn-wc-ledger-tools{flex-wrap:wrap}.pn-treasury-editor{width:100vw}.pn-treasury-settings{grid-template-columns:1fr}.pn-treasury-row{grid-template-columns:1fr!important}.pn-treasury-remove{grid-column:1}.pn-wc-chronicle-entry{grid-template-columns:1fr}.pn-wc-ledger-row{grid-template-columns:1fr}}
