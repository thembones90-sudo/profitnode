...
async function main() {
  ...
  try {
    ...
    const warChest=await evaluate(`(()=>{const content=document.querySelector('.pn-treasury'),sources=Array.from(document.querySelectorAll('[data-wc-source]')),icons=sources.map(x=>x.querySelector('img')),movement=document.querySelector('.pn-wc-movement-grid'),hero=document.querySelector('.pn-wc-hero'),fill=document.querySelector('.pn-wc-gauge-fill'),marker=document.querySelector('.pn-wc-gauge-floor span'),sourceLabel=document.querySelector('.pn-wc-source span');return{title:document.querySelector('h1').textContent,sources:sources.length,iconsLoaded:icons.every(x=>x.complete&&x.naturalWidth>0),currentHoard:document.body.textContent.includes('CURRENT HOARD'),fresh:document.body.textContent.includes('LAST RECOUNTED'),planningFx:document.body.textContent.includes('PLANNING FX'),reserveTitle:document.querySelector('.pn-wc-sources .pn-wc-section-head').textContent.trim(),movementCols:getComputedStyle(movement).gridTemplateColumns.split(' ').length,overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,ledgerRows:document.querySelectorAll('.pn-wc-ledger-row').length,ledgerEmpty:document.body.textContent.includes('NO MOVEMENTS'),ledgerFilters:document.querySelectorAll('[data-treasury-flow-filter]').length,runway:document.body.textContent.includes('RUNWAY'),surplus:!!document.querySelector('.pn-wc-surplus-protocol'),markerSize:parseFloat(getComputedStyle(marker).fontSize),sourceLabelSize:parseFloat(getComputedStyle(sourceLabel).fontSize),band:hero&&hero.dataset.wcBand,gauge:getComputedStyle(fill).backgroundImage}})()`);
    ...
    check("Fortress Reserve renders the BLOOD / YELLOW / GREEN scale and resolves €650 to YELLOW",warChest.band==='YELLOW'&&warChest.gauge.includes('rgb(166, 32, 45)')&&warChest.gauge.includes('rgb(208, 170, 50)')&&warChest.gauge.includes('rgb(87, 152, 92)'),JSON.stringify(warChest));
    ...
    check("MARK AS SOLD form completion records the sale and closes",directSale.modalClosed&&directSale.status==='SOLD'&&directSale.saleId&&directSale.completed,JSON.stringify(directSale));
    check("direct inventory sale realizes profit and leaves ACTIVE INVENTORY",directSale.profitDelta===4000&&directSale.flow===9000&&directSale.hidden,JSON.stringify(directSale));
    ...
    await evaluate(`document.querySelector('[data-close-modal]').click()`);
    const soldViewLink=await evaluate(`(()=>{state.route='inventory';state.filters.inventory.status='SOLD';render();document.querySelector('[data-open-entity="inventory"][data-id="${directSale.itemId}"]').click();const button=document.querySelector('[data-view-inventory-sale="${directSale.saleId}"]'),label=button&&button.textContent;button&&button.click();return {button:!!button,label:label,route:state.route,modalEntity:state.modal&&state.modal.entityType,modalId:state.modal&&state.modal.id}})()`);
    ...
    const paidUpdate=await evaluate(`(()=>{document.querySelector('[data-pb-quick-price-save="CPU"]').click();const item=Store.all('inventory').find(x=>x.model==='Core i7-6700'&&x.assignedProjectId===state.pbId),actual=document.querySelector('[data-pb-cost="actual"]'),planned=document.querySelector('[data-pb-cost="planned"]'),final=document.querySelector('[data-pb-cost="final"]');return {price:item&&item.purchasePrice,card:document.querySelector('[data-pb-slot="CPU"]').textContent,actual:actual&&actual.textContent,planned:planned&&planned.textContent,final:final&&final.textContent}})()`);
    check("paid edit refreshes the card and keeps actual/planned/final totals separate",paidUpdate.price===750&&paidUpdate.card.includes('750 RSD')&&paidUpdate.actual.includes('9.750 RSD')&&paidUpdate.planned.includes('7.000 RSD')&&paidUpdate.final.includes('16.750 RSD'),JSON.stringify(paidUpdate));
