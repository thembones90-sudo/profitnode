...
async function main() {
  ...
  try {
    ...
    const warChest=await evaluate(`(()=>{const content=document.querySelector('.pn-treasury'),sources=Array.from(document.querySelectorAll('[data-wc-source]')),icons=sources.map(x=>x.querySelector('img')),movement=document.querySelector('.pn-wc-movement-grid'),hero=document.querySelector('.pn-wc-hero'),fill=document.querySelector('.pn-wc-gauge-fill');return{title:document.querySelector('h1').textContent,sources:sources.length,iconsLoaded:icons.every(x=>x.complete&&x.naturalWidth>0),currentHoard:document.body.textContent.includes('CURRENT HOARD'),fresh:document.body.textContent.includes('LAST RECOUNTED'),movementCols:getComputedStyle(movement).gridTemplateColumns.split(' ').length,overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,ledgerRows:document.querySelectorAll('.pn-wc-ledger-row').length,band:hero&&hero.dataset.wcBand,gauge:getComputedStyle(fill).backgroundImage}})()`);
