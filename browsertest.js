...
async function main() {
  ...
  try {
    ...
    check("BUILD loadout renders canonical quality badges for owned and planned catalog components",buildUi.mobo.card==='UNCOMMON'&&buildUi.mobo.badge==='UNCOMMON'&&buildUi.cpu.card==='COMMON'&&buildUi.cpu.badge==='COMMON'&&buildUi.gpu.card==='COMMON'&&buildUi.gpu.badge==='COMMON'&&buildUi.badges===3&&buildUi.mobo.color!==buildUi.cpu.color&&buildUi.mobo.border!==buildUi.cpu.border,JSON.stringify(buildUi));
    await cdp.send("Emulation.setDeviceMetricsOverride",{width:720,height:900,deviceScaleFactor:1,mobile:false});
    const mobileBuildUi=await evaluate(`(()=>{const grid=document.querySelector('.pn-pb-grid');return {overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,columns:getComputedStyle(grid).gridTemplateColumns.split(' ').length,badges:document.querySelectorAll('[data-pb-loadout] [data-pb-quality-badge]').length}})()`);
    check("quality badges preserve the single-column BUILD loadout on mobile",!mobileBuildUi.overflow&&mobileBuildUi.columns===1&&mobileBuildUi.badges===3,JSON.stringify(mobileBuildUi));
    await cdp.send("Emulation.setDeviceMetricsOverride",{width:1920,height:1080,deviceScaleFactor:1,mobile:false});
