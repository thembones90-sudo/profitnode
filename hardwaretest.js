...
const results = env.run(sandbox, `(() => {
  ...
  out.push(['unreadable PSU wattage stays UNVERIFIED and never hard-fails the build', reserveModel.verdict==='PASS'&&reserveModel.counts.UNVERIFIED>0&&reserveModel.counts.FAIL===0&&reserveHtml.includes('>CHECK</span>')]);
  ...
  out.push(['reference build renders a calm Build Check dashboard', sanityHtml.includes('data-build-check-verdict="PASS"')&&!/data-bc-level="(WARN|FAIL)"/.test(sanityHtml)&&sanityHtml.includes('NOTES <b>2</b>')]);
