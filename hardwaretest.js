...
const results = env.run(sandbox, `(() => {
  ...
  const kingston2666={category:'RAM',manufacturer:'Kingston',model:'Fury Beast DDR4',ramType:'DDR4',totalCapacity:16,moduleCount:2,moduleCapacity:8,ramSpeedMTs:2666,casLatency:16,matchedKit:true};
  const kingston2666Tier=pnPartNameTier(kingston2666);
  out.push(['structured inventory RAM uses the live RAM engine instead of name-only fallback', kingston2666Tier.key==='UNCOMMON'&&kingston2666Tier.rating===52&&kingston2666Tier.source==='configuration']);
  const kingston2666Partial=pnPartNameTier(Object.assign({},kingston2666,{casLatency:null}));
  out.push(['partially specified 16GB 2x8 DDR4-2666 no longer collapses to POOR while CAS is unknown', kingston2666Partial.key==='COMMON'&&kingston2666Partial.rating>=30]);
  out.push(['legacy ramSpeedMHz inventory data migrates into MT/s semantics without losing the rating', pnPartNameTier(Object.assign({},kingston2666,{ramSpeedMTs:null,ramSpeedMHz:2666})).rating===52]);
  const kingstonRamForm=renderRamDetailsField(kingston2666);
  out.push(['inventory RAM form uses MT/s and keeps 2-module / 8GB-per-module state visible', kingstonRamForm.includes('SPEED (MT/s)')&&!kingstonRamForm.includes('SPEED (MHz)')&&kingstonRamForm.includes('<option value="2" selected>2 MODULES</option>')&&kingstonRamForm.includes('<option value="8" selected>8 GB</option>')]);
  out.push(['inventory RAM summary renders MT/s and the physical kit configuration', inventoryRamSummary(kingston2666).includes('2666 MT/s')&&inventoryRamSummary(kingston2666).includes('16GB (2×8GB)')]);
  const kingstonSkuDetected=detectInventoryRamDetails('Kingston Fury Beast DDR4 KF426C16BBK2/16',catalogFind('RAM','Kingston Fury Beast DDR4'));
  out.push(['exact RAM SKU resolves its real kit structure and timings', kingstonSkuDetected.totalCapacity===16&&kingstonSkuDetected.moduleCount===2&&kingstonSkuDetected.moduleCapacity===8&&kingstonSkuDetected.ramSpeedMTs===2666&&kingstonSkuDetected.casLatency===16&&kingstonSkuDetected.voltage==='1.2V'&&kingstonSkuDetected.partNumber==='KF426C16BBK2/16'&&kingstonSkuDetected.matchedKit===true]);
  const kingstonLegacyMigration={category:'RAM',manufacturer:'Kingston',model:'Fury Beast DDR4',ramType:'DDR4',totalCapacity:16,moduleCapacity:8,ramSpeedMTs:2666,casLatency:16};
  out.push(['legacy RAM record missing moduleCount infers 2 MODULES from 16GB total / 8GB per module', inventoryRamDetails(kingstonLegacyMigration).moduleCount===2&&renderRamDetailsField(kingstonLegacyMigration).includes('<option value="2" selected>2 MODULES</option>')&&renderRamDetailsField(kingstonLegacyMigration).includes('<option value="8" selected>8 GB</option>')]);
  out.push(['legacy RAM record missing moduleCount still rates via structured engine, never POOR', pnPartNameTier(kingstonLegacyMigration).key!=='POOR'&&pnPartNameTier(kingstonLegacyMigration).rating===52]);
  ...
  out.push(['RIG BUILD renders the complete V3.1 DDR4 configuration fields', ramEditor.includes('data-rig-ram-field="technology"')&&ramEditor.includes('data-rig-ram-field="dimmType"')&&ramEditor.includes('data-rig-ram-field="moduleCount"')&&ramEditor.includes('<option value="3"')&&ramEditor.includes('data-rig-ram-field="speed"')&&ramEditor.includes('data-rig-ram-field="casLatency"')&&ramEditor.includes('data-rig-ram-field="expo"')&&ramEditor.includes('data-rig-ram-field="voltage"')&&ramEditor.includes('data-rig-ram-field="partNumber"')&&ramEditor.includes('data-rig-ram-field="matchedKit"')&&!ramEditor.includes('data-rig-ram-field="model"')]);
  ...
  out.push(['PARTS VAULT tier pill appears beside part names that clear COMMON', pvHtml.includes('<span class="pn-inv-tier-chip pn-part-name-rare">RARE</span>')&&!pvHtml.includes('pn-inv-tier-chip undefined')]);
