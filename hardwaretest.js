...
const results = env.run(sandbox, `(() => {
  ...
  out.push(['active AM5 tiers match the PN tier mapping', canonicalHardware.cpus.filter(c=>c.platform==='AM5'&&c.availability_status==='RELEASED').every(c=>Math.floor(c.pn_score/2)>=0&&(c.pn_tier==='SCRAPBLADE'||c.pn_tier==='SCRAPWRAITH'||c.pn_tier==='REVENANT'||c.pn_tier==='GHOUL'||c.pn_tier==='ALGHOUL'))]);
  out.push(['AM5 tier boundaries honor the canonical scale', canonicalHardware.cpus.filter(c=>c.platform==='AM5'&&c.availability_status==='RELEASED').every(c=>{const s=c.pn_score;const t=c.pn_tier;return(t==='ALGHOUL'&&s>=80&&s<=100)||(t==='GHOUL'&&s>=60&&s<80)||(t==='REVENANT'&&s>=45&&s<60)||(t==='SCRAPWRAITH'&&s>=30&&s<45)||(t==='SCRAPBLADE'&&s>=0&&s<30)})]);
  ...
  out.push(['AM5 registry exposes the canonical gameplay anchors', canonicalHardware.cpus.find(c=>c.model==='Ryzen 7 7800X3D').pn_score===91&&canonicalHardware.cpus.find(c=>c.model==='Ryzen 7 7800X3D').pn_tier==='ALGHOUL']);
  ...
  out.push(['every ASUS AM5 board resolves to a valid tier', ['SCRAPBLADE','SCRAPWRAITH','REVENANT','GHOUL','ALGHOUL'].every(t=>asusAm5.some(b=>b.pn_tier===t))||asusAm5.every(b=>['SCRAPBLADE','SCRAPWRAITH','REVENANT','GHOUL','ALGHOUL'].includes(b.pn_tier))]);
