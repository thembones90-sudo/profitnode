...
const results = env.run(sandbox, `(() => {
  ...
  out.push(['Build Check PASS + UNVERIFIED resolves PASS, never warnings', unverifiedModel.verdict==='PASS'&&unverifiedModel.counts.PASS>0&&unverifiedModel.counts.UNVERIFIED>0&&unverifiedModel.counts.WARN===0&&unverifiedModel.counts.FAIL===0]);
  ...
  out.push(['contradictory Build Check severity states are impossible', scenarios.every(([,rig])=>{const model=buildCheckModel(rig),expected=model.counts.FAIL?'FAIL':model.counts.WARN?'WARNING':model.findings.length?'PASS':'UNVERIFIED';return model.verdict===expected} )]);
  ...
  out.push(['COPY BUILD export embeds Build Check statuses and the shop read', plainTxt.includes('BUILD CHECK:')&&plainTxt.includes('VERDICT: COMPATIBLE')&&plainTxt.includes('COMPATIBLE')&&plainTxt.includes('SHOP READ:')&&plainTxt.includes('READY TO ASSEMBLE')&&plainTxt.includes('NOTES:')]);
