...
const results = env.run(sandbox, `(() => {
  ...
  out.push(['contradictory severity states stay impossible against a socket-clash build', (function(){const r=goodRig();r.slots.CPU=cat2('CPU','AMD Ryzen 5 7600');r.slots.MOBO=cat2('MOBO','ASRock B550M-HDV');const m=buildCheckModel(r);return m.verdict==='FAIL'&&m.findings.some(f=>f.id==='CORE_CPU_SOCKET'&&f.status==='FAIL')})()]);
  ...
  out.push(['socket clash is a genuine incompatibility', clashModel.findings.some(f=>f.id==='CORE_CPU_SOCKET'&&f.status==='FAIL')&&clashModel.verdict==='FAIL']);
  out.push(['socket clash routes into the Build Integrity tile with visible FAIL detail', (function(){const seg=clashHtml.slice(clashHtml.indexOf('data-bc-tile="build-integrity"'),clashHtml.indexOf('data-bc-tile="psu"')),levels=[...seg.matchAll(/data-bc-level="([^"]+)"/g)].map(m=>m[1]);return seg.includes('Build Integrity')&&levels.includes('FAIL')&&clashHtml.includes('CPU platform')})()]);
  const verdicts=scenarios.map(([,rig])=>{state.rigDraft=rig;const model=buildCheckModel(rig),html=renderBuildCheck(),tileLevels=[...html.matchAll(/pn-bc-chip pn-bc-status" data-bc-level="([^"]+)"/g)].map(m=>m[1]),worstTiles=tileLevels.reduce((a,b)=>Math.max(a,BC_SEVERITY[bcStatus(b)]),0),worstFindings=Math.max(...model.findings.map(f=>BC_SEVERITY[bcStatus(f.status)]),0),expected=worstFindings===3?'FAIL':worstFindings===2?'WARNING':worstFindings===1?'UNVERIFIED':'PASS';return tileLevels.length===5&&model.verdict===expected&&worstTiles===worstFindings});
  ...
  out.push(['unreadable PSU wattage stays UNVERIFIED and never hard-fails the build', reserveModel.verdict==='UNVERIFIED'&&reserveModel.counts.FAIL===0&&reserveHtml.includes('>CHECK</span>')]);
