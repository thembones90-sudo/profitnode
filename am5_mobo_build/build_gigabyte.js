"use strict";
/* PROFITNODE GIGABYTE AM5 build pipeline.
   Loads seed -> cross-checks every SKU against the official GIGABYTE AM5
   product list (am5_registry_discovery_v1.json) -> assembles full records
   (UNKNOWN defaults) -> dedupes -> rates with PN_AM5_MOBO_V2 -> merges into
   production board catalog (replaces the 5 legacy Gigabyte AM5 rows) ->
   writes standalone registry. Prints completeness / rating / outlier audits. */

const fs = require("fs");
const path = require("path");
const { boards } = require("./gigabyte_seed.js");
...
const REG = path.join(ROOT, "profitnode_am5_motherboard_registry_gigabyte_v1.json");
...
/* official GIGABYTE AM5 SKU list from discovery */
const discovery = JSON.parse(fs.readFileSync(DISCOVERY, "utf8"));
const gigaProducts = (discovery.manufacturers.Gigabyte && discovery.manufacturers.Gigabyte.products) || [];
const DISC = new Map();
gigaProducts.forEach(p => { const k = pnNorm(p.slug).replace(/\s*rev\s*$/, ""); DISC.set(k, p); });
...
function featuresString(r){
  ...
  else if (/WIFI\s*5\b/i.test(s.wifi_standard || "")) out.push("Wi-Fi 5");
  ...
  if (/STEALTH|PROJECT STEALTH/.test(r.model || "")) out.push("STEALTH back-connect");
  return out.join(" · ") || UNK;
}
...
/* ---- discovery coverage check (bidirectional) ---- */
const seedSlugs = new Set();
boards().forEach(({ rec }) => seedSlugs.add(pnNorm(rec.m)));
const missingFromSeed = [...DISC.keys()].filter(k => !seedSlugs.has(k));
const notInDiscovery = [...seedSlugs].filter(k => !DISC.has(k));
if (missingFromSeed.length || notInDiscovery.length) {
  console.log("DISCOVERY MISMATCH — official GIGABYTE AM5 slugs not in seed:", missingFromSeed);
  console.log("           seed entries not matched to discovery:", notInDiscovery);
  process.exit(1);
}
console.log("discovery cross-check OK:", DISC.size, "official GIGABYTE AM5 SKUs all covered by seed.");

const raw = boards();
const assembled = raw.map(({ chipset, family, rec }) => {
  const m = rec.m;
  const canon = m.replace(/\s+REV\.?\s*\d+(\.\d+)?\b/i, "").trim();
  const slug = pnNorm(m).replace(/ /g, "-");
  const disc = DISC.get(pnNorm(m));
  const recBase = Object.assign({}, rec);
  delete recBase.m;
  const record = {
    id: "gigabyte-" + slug,
    manufacturer: "Gigabyte", brand: "Gigabyte", family, model: m, full_model_name: "Gigabyte " + m,
    manufacturer_part_number: UNK, socket: "AM5", chipset,
    form_factor: /\b(X870M|B850M|B650EM|B650M|B840M|A620M|A620AM)\b/i.test(m) ? "Micro-ATX"
      : /\b(X870I|B850I|B650I|A620I)\b/i.test(m) ? "Mini-ITX" : ff(recBase.form_factor || UNK),
    canonical_model: canon, revision: null, revisions: [], color_variant: /-ICE$|-ICE\b|ICE/i.test(m) ? "ICE" : null,
    ...
    source_url: disc && disc.source_urls && disc.source_urls[0]
      ? disc.source_urls[0] : "https://www.gigabyte.com/search?keyword=" + encodeURIComponent(m),
...
console.log("== GIGABYTE AM5 build ==");
...
console.log("family    :", JSON.stringify(Object.fromEntries(Object.entries(byFamily).sort((a, b) => b[1] - a[1]))));
...
fs.writeFileSync(REG, JSON.stringify({ scope: "Gigabyte AM5 (phase 3 of AM5 Motherboard Foundation v2)", generated: TODAY, rating_method: "PN_AM5_MOBO_V2", count: rated.length, boards: rated }, null, 2));

/* ---- merge into production catalog ---- */
const cat = JSON.parse(fs.readFileSync(CAT, "utf8"));
const before = cat.boards.length;
cat.boards = cat.boards.filter(b => !(String(b.socket || "") === "AM5" && String(b.brand || "").toLowerCase() === "gigabyte"));
const dropped = before - cat.boards.length;
cat.boards.push(...rated);
fs.writeFileSync(CAT, JSON.stringify(cat, null, 2));
console.log("\nproduction catalog:", before, "->", cat.boards.length, "( dropped", dropped, "legacy Gigabyte AM5 rows )");
