"use strict";
/* PROFITNODE BIOSTAR AM5 build pipeline.
   ...
   (replaces any BIOSTAR AM5 rows) -> writes standalone registry.
   Every SKU maps to its official BIOSTAR introduction.php?S_ID= page
   (verified from the official AM5 CPU support list, 2026-09). Prints audits. */

const fs = require("fs");
const path = require("path");
const { boards } = require("./biostar_seed.js");
...
const REG = path.join(ROOT, "profitnode_am5_motherboard_registry_biostar_v1.json");
...
/* official BIOSTAR AM5 product S_IDs (from the official AM5 CPU support list) */
const SID = { "B840MT-E": 1182, "B850M-SILVER": 1163, "B850MS-E": 1208, "B850MT-E PRO": 1197,
  "B850MT2-E DJ": 1177, "X870E VALKYRIE": 1161, "A620MH AURORA": 1152, "A620MHC": 1184,
  "A620MS-E": 1183, "A620MT-E 2.0": 1203, "A620MP-E PRO": 1117, "A620MS": 1153, "A620MT": 1129,
  "B650M-SILVER": 1091, "B650M X3D PRO+": 1178, "B650MP-E PRO": 1116, "B650MS2": 1136,
  "B650MS2-E": 1154, "B650MT": 1135, "B650MT-E PRO": 1142, "X670E VALKYRIE": 1079, "B650EGTQ": 1090 };
...
const assembled = raw.map(({ chipset, family, rec }) => {
  ...
  const record = {
    id: "biostar-" + slugBase,
    manufacturer: "BIOSTAR", brand: "Biostar", family, model: m, full_model_name: "Biostar " + m,
    ...
    source_url: "https://www.biostar.com.tw/app/en/mb/introduction.php?S_ID=" + (SID[m] || ""),
...
fs.writeFileSync(REG, JSON.stringify({ scope: "BIOSTAR AM5 motherboard registry (PN_AM5_MOBO_V2)", generated: TODAY, rating_method: "PN_AM5_MOBO_V2", count: rated.length, boards: rated }, null, 2));

/* ---- merge into production catalog ---- */
const cat = JSON.parse(fs.readFileSync(CAT, "utf8"));
const before = cat.boards.length;
cat.boards = cat.boards.filter(b => !(String(b.socket || "") === "AM5" && String(b.brand || "").toLowerCase() === "biostar"));
const dropped = before - cat.boards.length;
cat.boards.push(...rated);
fs.writeFileSync(CAT, JSON.stringify(cat, null, 2));
console.log("\nproduction catalog:", before, "->", cat.boards.length, "( dropped", dropped, "legacy BIOSTAR AM5 rows )");
