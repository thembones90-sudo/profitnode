...
const REG = path.join(ROOT, "profitnode_am5_motherboard_registry_msi_v1.json");
...
function featuresString(r){
  ...
  if (/PROJECT ZERO|BTF|PZ WIFI/.test(r.model || "")) out.push("BTF back-connect");
  return out.join(" · ") || UNK;
}
...
const assembled = raw.map(({ chipset, family, rec }) => {
  ...
  delete recBase.m;
  const record = {
    id: "msi-" + pnNorm(m).replace(/ /g, "-"),
    manufacturer: "MSI", brand: "MSI", family, model: m, full_model_name: "MSI " + m,
    manufacturer_part_number: UNK, socket: "AM5", chipset, form_factor: /\b(A620M|B840M|B850M|B650M|X670M)\b/i.test(m) ? "Micro-ATX" : ff(recBase.form_factor || UNK),
    ...
    source_url: "https://www.msi.com/search?keyword=" + encodeURIComponent(m),
...
fs.writeFileSync(REG, JSON.stringify({ scope: "MSI AM5 (phase 2 of AM5 Motherboard Foundation v2)", generated: TODAY, rating_method: "PN_AM5_MOBO_V2", count: rated.length, boards: rated }, null, 2));

/* ---- merge into production catalog ---- */
const cat = JSON.parse(fs.readFileSync(CAT, "utf8"));
const before = cat.boards.length;
cat.boards = cat.boards.filter(b => !(String(b.socket || "") === "AM5" && String(b.brand || "").toLowerCase() === "msi"));
const dropped = before - cat.boards.length;
cat.boards.push(...rated);
fs.writeFileSync(CAT, JSON.stringify(cat, null, 2));
console.log("\nproduction catalog:", before, "->", cat.boards.length, "( dropped", dropped, "legacy MSI AM5 rows )");
