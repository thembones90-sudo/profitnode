"use strict";
/* PROFITNODE COLORFUL AM5 build pipeline.
   Loads seed -> assembles full records (UNKNOWN defaults) -> dedupes ->
   rates with PN_AM5_MOBO_V2 -> merges into the production board catalog
   (replaces any COLORFUL AM5 rows) -> writes standalone registry.
   source_url points at the official Colorful product-listing page for the
   board's series (iGame / CVN / BATTLE-AX / COLORFIRE). Prints audits. */

const fs = require("fs");
const path = require("path");
const { boards } = require("./colorful_seed.js");
...
const REG = path.join(ROOT, "profitnode_am5_motherboard_registry_colorful_v1.json");
...
/* official Colorful series product-listing pages (productlist?mid=84 = motherboards) */
const SERIES_URL = {
  "iGame": "https://www.colorfulgroup.com/en/home/productlist?mid=84&categoryids=3002",
  "CVN": "https://www.colorfulgroup.com/en/home/productlist?mid=84&categoryids=3377",
  "BATTLE-AX": "https://www.colorfulgroup.com/en/home/productlist?mid=84&categoryids=3420",
  "COLORFIRE": "https://www.colorfulgroup.com/en/home/productlist?mid=84&categoryids=3421"
};
const DEFAULT_URL = "https://www.colorfulgroup.com/en/home/productlist?mid=84";
...
const assembled = raw.map(({ chipset, family, rec }) => {
  ...
  const record = {
    id: "colorful-" + slugBase,
    manufacturer: "Colorful", brand: "Colorful", family, model: m, full_model_name: "Colorful " + m,
    ...
    source_url: SERIES_URL[family] || DEFAULT_URL,
...
console.log("== COLORFUL AM5 build ==");
...
fs.writeFileSync(REG, JSON.stringify({ scope: "COLORFUL AM5 motherboard registry (PN_AM5_MOBO_V2)", generated: TODAY, rating_method: "PN_AM5_MOBO_V2", count: rated.length, boards: rated }, null, 2));

/* ---- merge into production catalog ---- */
const cat = JSON.parse(fs.readFileSync(CAT, "utf8"));
const before = cat.boards.length;
cat.boards = cat.boards.filter(b => !(String(b.socket || "") === "AM5" && String(b.brand || "").toLowerCase() === "colorful"));
const dropped = before - cat.boards.length;
cat.boards.push(...rated);
fs.writeFileSync(CAT, JSON.stringify(cat, null, 2));
console.log("\nproduction catalog:", before, "->", cat.boards.length, "( dropped", dropped, "legacy COLORFUL AM5 rows )");
