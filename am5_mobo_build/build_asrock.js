"use strict";
/* PROFITNODE ASRock AM5 build pipeline.
   Loads seed -> cross-checks every SKU against the official ASRock AM5
   product list (am5_asrock_discovery_v1.json) -> assembles full records
   (UNKNOWN defaults) -> dedupes -> rates with PN_AM5_MOBO_V2 -> merges into
   production board catalog (replaces the 3 legacy ASRock AM5 rows) ->
   writes standalone registry. Prints completeness / rating / outlier audits. */

const fs = require("fs");
const path = require("path");
const { boards } = require("./asrock_seed.js");
...
const DISCOVERY = path.join(ROOT, "am5_asrock_discovery_v1.json");
const REG = path.join(ROOT, "profitnode_am5_motherboard_registry_asrock_v1.json");
...
/* official ASRock AM5 SKU list from discovery */
const discoveryData = JSON.parse(fs.readFileSync(DISCOVERY, "utf8"));
const DISC = new Map();
(discoveryData.rows || []).forEach(r => {
  const key = pnNorm(r.model || "");
  DISC.set(key, r);
});
...
/* ---- discovery coverage check (all discovery models must exist in seed) ---- */
...
if (missingFromSeed.length) {
  console.log("DISCOVERY MISMATCH — official ASRock AM5 slugs not in seed:", missingFromSeed);
  process.exit(1);
}
console.log("discovery cross-check OK:", DISC.size, "official ASRock AM5 SKUs all covered by seed.");
...
const assembled = raw.map(({ chipset, family, rec }) => {
  ...
  let formFactor = ff(recBase.form_factor || UNK);
  if (disc && disc.cpu_form_factor) {
    const raw = String(disc.cpu_form_factor).replace(/[()]/g, "").trim();
    if (/E-ATX|EATX/i.test(raw)) formFactor = "E-ATX";
    else if (/MATX|mATX/i.test(raw)) formFactor = "Micro-ATX";
    else if (/MITX|mITX|MINI-ITX/i.test(raw)) formFactor = "Mini-ITX";
    else if (/ATX/i.test(raw)) formFactor = "ATX";
  }

  const record = {
    id: "asrock-" + slug,
    manufacturer: "ASRock", brand: "ASRock", family, model: m, full_model_name: "ASRock " + m,
    ...
    source_url: disc && disc.source_url ? disc.source_url : "https://www.asrock.com/mb/AMD/" + encodeURIComponent(m) + "/index.asp",
...
fs.writeFileSync(REG, JSON.stringify({ scope: "ASRock AM5 (phase 3 of AM5 Motherboard Foundation v2)", generated: TODAY, rating_method: "PN_AM5_MOBO_V2", count: rated.length, boards: rated }, null, 2));

/* ---- merge into production catalog ---- */
const cat = JSON.parse(fs.readFileSync(CAT, "utf8"));
const before = cat.boards.length;
cat.boards = cat.boards.filter(b => !(String(b.socket || "") === "AM5" && String(b.brand || "").toLowerCase() === "asrock"));
const dropped = before - cat.boards.length;
cat.boards.push(...rated);
fs.writeFileSync(CAT, JSON.stringify(cat, null, 2));
console.log("\nproduction catalog:", before, "->", cat.boards.length, "( dropped", dropped, "legacy ASRock AM5 rows )");
